...
{
class MP_Dashboard_recent_archives extends MP_WP_Dashboard_widget_
{
...
<?php 			
		while ( $mp_mails->have_mails() ) : $mp_mails->the_mail();
			$args = array( 'id' => $mp_mails->get_the_ID(), 'action' => 'mp_ajax', 'mp_action' => 'iview', 'TB_iframe' => 'true' );
			$view_url = esc_url( add_query_arg( $args, admin_url( 'admin-ajax.php' ) ) );    
?>
	<li id="mail-<?php $mp_mails->the_ID(); ?>">
		<h4 style="font-weight:normal;">
			<a class="thickbox thickbox-preview" title="<?php _e( 'View', 'MailPress' ); ?>" href="<?php echo $view_url; ?>" >
...
<p class="textright">
	<a class="button" href="<?php echo esc_url( add_query_arg( 'status', archived, MailPress_mails ) ); ?>">
		<?php _e( 'View all' ); ?>
	</a>
</p>
<?php
...
new MP_Dashboard_recent_archives( __( 'MailPress - Recent Archives', 'MailPress' ) );
}