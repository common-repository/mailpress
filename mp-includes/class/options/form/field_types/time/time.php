...
class MP_Form_field_type_time extends MP_form_field_type_
{
	...
	function get_name( $field )
	{
		...
		return $this->prefix.'['.$this->field->form_id . ']['. $this->field->id . ']' .  ( ( isset( $this->field->settings['options']['is'] ) ) ? ( '[' . ( ( $this->field->settings['options']['is'] == 'am_pm' ) ? $this->prefix . $this->field->settings['options']['is'] : $this->field->settings['options']['is'] ) . ']' ) : '' ) ;
	}
	...
	function get_id( $field )
	{
		...
		return $this->prefix  .  $this->field->form_id . '_' . $this->field->id .        ( ( isset( $this->field->settings['options']['is'] ) ) ? ( '_' . ( ( $this->field->settings['options']['is'] == 'am_pm' ) ? $this->field->settings['attributes']['value']           : $this->field->settings['options']['is'] )       ) : '' ) ;
	}
	...
	function submitted( $field )
	{
		...
		$format = $this->field->settings['options']['mail_time_format'];
		if ( empty( $format ) ) $format = get_option( 'time_format' );

		$this->field->submitted['value'] = $value;
		$this->field->submitted['text']  = date( $format, mktime( $value['h'], $value['mn'] ) );
		if ( isset( $value[$this->prefix . 'am_pm'] ) ) $this->field->submitted['text'] .= ' '  . $value[$this->prefix . 'am_pm'];
		if ( isset( $value['tz'] ) ) 				$this->field->submitted['text'] .= ' ( ' . $value['tz'] . ' )'; 
	...
	function attributes_filter( $no_reset )
	{
		...
		if ( '1' == $this->field->settings['options']['form_time_init_value'] )
		{
			...
			if ( $x = MP_Ip::get_latlng( filter_input( INPUT_SERVER, 'REMOTE_ADDR' ) ) )
			{
...
// timezones
		...
		$this->field->settings['options']['value'] = $this->get_value();
