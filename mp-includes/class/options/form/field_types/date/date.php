...
class MP_Form_field_type_date extends MP_form_field_type_
{
	...
	function get_name( $field )
	{
		...
		return $this->prefix.'['.$this->field->form_id . ']['. $this->field->id . ']' . ( ( isset( $this->field->settings['options']['is'] ) ) ? '[' . $this->field->settings['options']['is'] . ']' : '' );
	}
	...
	function get_id( $field )
	{
		...
		return $this->prefix  .  $this->field->form_id . '_' . $this->field->id .       ( ( isset( $this->field->settings['options']['is'] ) ) ? '_' . $this->field->settings['options']['is'] : '' );
	}
	...
	function submitted( $field )
	{
		...
		$format = $this->field->settings['options']['mail_date_format'];
		if ( empty( $format ) ) $format = get_option( 'date_format' );

		$this->field->submitted['value'] = $value;
		$this->field->submitted['text']  =  mysql2date( $format, mktime( 0, 0, 0, $value['m'], $value['d'], $value['y'] ) );
