...
// for wp admin
		if ( is_admin() && class_exists( 'MailPress_mailinglist' ) && current_user_can( 'MailPress_manage_mailinglists' ) ) add_filter( 'MailPress_scripts', array( __CLASS__, 'scripts' ), 8, 2 );
	}
	...
	public static function form( $url_parms )
	{
		...
		if ( class_exists( 'MailPress_mailinglist' ) && current_user_can( 'MailPress_manage_mailinglists' ) ) $out .= MP_Mailinglist::dropdown( array( 'echo' => false, 'htmlname' => 'bulk_import_mailinglist', 'htmlid' => 'bulk_import_mailinglist', 'hierarchical' => true, 'orderby' => 'name', 'hide_empty' => '0', 'show_option_none' => __( 'Choose mailinglist', 'MailPress' ) ) ) . "\r\n";
	...
	public static function bulk_users( $mails, $type )
	{
		...
		foreach ( $email_array as $email_name ) 
		{
			...
			if ( MailPress::is_email( $email ) && ( 'deleted' == MP_User::get_status_by_email( $email ) ) )
			{
				...
				if ( class_exists( 'MailPress_mailinglist' ) && current_user_can( 'MailPress_manage_mailinglists' ) )
				{
